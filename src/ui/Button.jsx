...
export default function Button({ children, disabled, to, type, onClick }) {
  ...
  const styles = {
    ...
    secondary:
      "inline-block text-sm rounded-full border-2 border-stone-300 px-4 py-2.5 md:px-6 md:py-3.5 font-semibold uppercase tracking-wide text-stone-400 transition-colors duration-300 hover:bg-stone-300 hover:text-stone-800 focus:text-stone-800 focus:outline-none focus:bg-stone-300 focus:ring focus:ring-stone-200 focus:ring-offset-2 disabled:cursor-not-allowed sm:px-6 sm:py-4",
  };
  if (to)
    return (
